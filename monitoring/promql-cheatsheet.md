...
## What Each Query Teaches You

### 1. `up`
- Every scrape target exposes an `up` metric set to `1` on successful scrapes.
- Use it to prove connectivity instantly; if any value is `0`, jump to the service logs.

### 2. `up{job="bookstore"}`
- Adds a label selector (`job="bookstore"`) so you only see the services from this stack.
...
- Histograms automatically create `_count`; using `rate()` turns that into requests per second.
...
- Same as #3 but for the Next.js histogram name. Shows the UI traffic.
...
- Rules: mention how alerts appear here (alert-rules.yml is already configured).
- Status -> Configuration: show `scrape_configs` to prove manual service discovery.
- Status -> TSDB Status: optional, demonstrates Prometheus internals (storage size, churn).
