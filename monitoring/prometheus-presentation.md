...
Run `docker compose ps` to confirm every container is Up.
...
Grafana is now included in the default compose file. Access it at http://localhost:3030 (admin/admin). Dashboards are pre-provisioned automatically.
...
## 9. PromQL & Graph UI Reference

| Goal | PromQL | Notes |
|------|--------|-------|
| Show all targets UP | `up` | value 1 per job/instance when healthy |
| Per-service request rate | `sum by (instance) (rate(http_request_duration_seconds_count[1m]))` | swap metric for frontend on Next.js |
...
| Error ratio | `sum(rate(http_requests_total{status=~"4..|5.."}[5m])) / sum(rate(http_requests_total[5m]))` | requires `http_requests_total` counter |
...
- Add recording rules & alerts (`monitoring/alert-rules.yml` already included).
